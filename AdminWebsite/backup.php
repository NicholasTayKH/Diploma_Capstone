<?php
    ...
    if(isset($_GET["confirmSave"])){
        $foodid = $_GET["confirmSave"];
        $foodname = $_GET["foodName"];
        $fooddesc = $_GET["foodDescription"];
        $foodcat = $_GET["cat"];
        $foodprice = $_GET["foodPrice"];
        $image = file_get_contents($_FILES['img']['tmp_name']);
        $imageData = mysqli_real_escape_string($connn, $image);
        $editSql = "UPDATE food SET foodname='$foodname', foodcategory='$foodcat', fooddescription='$fooddesc', foodprice='$foodprice', foodimage='$imageData' WHERE foodid='$foodid'";
    ...
    if(isset($_GET["newConfirmSave"])){
        $newName = $_GET["newFoodName"];
        $newCat = $_GET["newCat"];
        $newDesc = $_GET["newFoodDescription"];
        $newPrice = $_GET["newFoodPrice"];
        $image = file_get_contents($_FILES['img']['tmp_name']);
        $imageData = mysqli_real_escape_string($connn, $image);

        $newSql = "INSERT INTO food(foodname,foodcategory,fooddescription,foodprice,foodimage)
                    VALUES ('$newName','$newCat','$newDesc','$newPrice','$imageData')";
        ...
        <?php
            if(isset($_GET["edit"])){
                ...
                <div id=\"overlay\" class=\"hidden\">
                    <div class=\"overlay-content-edit\">
                        ...
                        <form action=\"item.php\" method=\"get\" enctype=\"multipart/form-data\">   
            ...
            if(isset($_GET["addItem"])){
                ...
                echo"       </select>
                            ...
                            <select name=\"newCat\" placeholder=\"Please Select A Stock\"required>
                ...
                if (mysqli_num_rows($resultSearchSql) > 0) {
                    ...
                    <div id=\"overlay\" class=\"hidden\" onclick=\"history.back();\">
                        <div class=\"overlay-content-image\">
                            <img id=\"tableimg\" src=\"data:Images/png;base64,". base64_encode($rowSearch['foodimage']) ."\" style=\"width:500px; height:700px;\">
                        </div>
                    </div>
                    ";
                }
